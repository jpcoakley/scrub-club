...
- Schedule Beer Man requests: the Request button posts to `apps-script/beer-request.gs`, deployed as a web app from the roster sheet. Its URL goes in `BEER_REQUEST_URL` in index.html (empty hides the buttons). It only accepts names already on the roster tab, only for unplayed games with no Beer Man yet, and writes Beer Duty on the sheet's Schedule tab. Editing the script means redeploying it in Apps Script (Manage deployments, new version) so the URL stays the same.
